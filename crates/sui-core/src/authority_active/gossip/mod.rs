...
#[async_trait]
impl<A> FollowerDigestHandler<A> for GossipDigestHandler
where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    async fn handle_digest(&self, follower: &Follower<A>, digest: ExecutionDigests) -> SuiResult {
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn start<Handler: FollowerDigestHandler<A>>(
        self,
        duration: Duration,
        handler: Handler,
    ...
    async fn follow_peer_for_duration<'a, Handler: FollowerDigestHandler<A>>(
        &self,
        duration: Duration,
        handler: Handler,
    ) -> SuiResult {
